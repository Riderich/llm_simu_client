"""
RECAP 合作样本 CBS 2-8 分类 - 大模型 API 版
使用 qwen-turbo 与 ESConv/MESC 保持一致
"""
import os
import json
import time
...
OUTPUT_FILE = "workspace/results/recap_coop_cbs_api.json"
SAVE_EVERY = 50
...
def format_context(dialogue, max_turns=3):
    ...
    for turn in dialogue[-max_turns*2:]:
        role = turn.get("role", "")
        content = turn.get("content", "").strip()[:80]
        if role and content:
...
def main():
    ...
    print(f"预计耗时: {total * 0.5:.0f} 秒 ({total * 0.5 / 60:.1f} 分钟)")
    print("-" * 60)
    
    for i, sample in enumerate(coop_samples):
        cbs = classify_with_api(sample)
        ...
        # 进度显示
        if (i + 1) % 10 == 0:
            progress = (start_idx + i + 1) / 10
            print(f"  进度: {start_idx + i + 1}/1000 ({progress:.1f}%) - 最新: {cbs}")
    ...
    for cbs_code, name in labels_map.items():
        cbs_full = f"{cbs_code}-{name}"
        cnt = cats.get(cbs_full, 0)
        pct = cnt / len(results) * 100
        print(f"  {cbs_full}: {cnt:4d} ({pct:5.1f}%)")
    
    unknown = cats.get("Unknown", 0) + cats.get("Error", 0) + cats.get("Error-", 0)
    if unknown > 0:
        print(f"  Unknown/Error: {unknown:4d}")
