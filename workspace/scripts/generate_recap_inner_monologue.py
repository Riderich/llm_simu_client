"""
RECAP 中文 Inner Monologue 生成脚本
基于 5 套中文提示词模板，对 RECAP 数据集生成第一人称内心独白
"""
...
def select_template(item):
    """根据 resistance_label 选择对应的中文模板，返回 (template, subtype_str)"""
...
def main():
    parser = argparse.ArgumentParser(description="RECAP 中文 Inner Monologue 生成")
    parser.add_argument("--input", default="dataset/RECAP.json")
    parser.add_argument("--output", default="results/recap_inner_monologue.json")
    ...
    subset = raw_data[:args.num_cases] if args.num_cases else raw_data
    print(f"总样本数：{len(subset)}，workers={args.workers}，model={args.model}")

    # 初始化 results（加 _recap_idx 和 inner_monologue 字段）
    results = []
    for i, item in enumerate(subset):
        r = dict(item)
        r["_recap_idx"] = i
        r["inner_monologue"] = None
        results.append(r)
    ...
    if args.resume and os.path.exists(output_path):
        with open(output_path, "r", encoding="utf-8") as f:
            saved = json.load(f)
        for item in saved:
            im = item.get("inner_monologue", "")
            idx = item.get("_recap_idx")
            if idx is not None and im and not im.startswith("ERROR:") and im != "[SKIPPED - too short]":
                existing_map[idx] = im
        for idx, im in existing_map.items():
            if idx < len(results):
                results[idx]["inner_monologue"] = im
        print(f"[resume] 已恢复 {len(existing_map)} 条，跳过")

    to_process = [(i, subset[i]) for i in range(len(subset)) if i not in existing_map]
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(generate_one, idx, item, inference, args.model, args.max_retries): idx
        ...
        for future in as_completed(futures):
            ...
            is_ok = im and not im.startswith("ERROR:")
            success += int(is_ok)
            failed += int(not is_ok)
            status = "✓" if is_ok else "✗"

            target_preview = results[idx].get("target_utterance", "")[:30]
            print(f"[{processed}/{len(to_process)}] idx={idx} {status} | {target_preview}")
            if is_ok:
                print(f"  → {im[:80]}")
            else:
                print(f"  → {im}")

            if processed % args.save_every == 0 or processed == len(to_process):
