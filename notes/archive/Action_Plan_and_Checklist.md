...
- [x] 数据集调研与入库（ESConv, MESC, ExTES, AnnoMI, RECAP）
- [x] 确定 Inner Monologue "三段式"链条架构
- [x] 设计出并改良了带有 5 套专属约束体系的第一人称沉浸式模板

#### 1.2 细粒度标注完成 ✅ (2026-03-26)
- [x] **ESConv 阻抗细粒度标注**：5,609条（PsyFIRE 13类，100%准确率）
- [x] **ESConv 合作 CBS 分类**：13,763条（CBS 2-8）
- [x] **MESC 阻抗细粒度标注**：1,608条（PsyFIRE 13类，100%准确率）
- [x] **MESC 合作 CBS 分类**：16,828条（CBS 2-8）
- [x] **RECAP 数据处理**：5,154条（含E1/E2/E3细分）
- [x] **AnnoMI IM 生成**：153条（验证完成）
- [x] 质量修复与标签清洗（解决#注释、无效组合、格式问题等）

**一阶段统计**：
| 数据集 | 阻抗样本 | 合作样本 | 状态 |
|--------|---------|---------|------|
| AnnoMI | 30 | 123 | ✅ IM生成完成 |
| ESConv | 5,609 | 13,763 | ✅ 细粒度标注完成 |
| MESC | 1,608 | 16,828 | ✅ 细粒度标注完成 |
| RECAP | 4,154 | 1,000 | ✅ 预处理完成 |
| **合计** | **~11,400** | **~31,700** | ✅ **阶段一完成** |

---

### 阶段二：Inner Monologue 生成 🔄 进行中

#### 2.1 英文数据集 IM 生成
- [ ] **ESConv IM 生成**：基于细粒度标注结果，使用5套模板
- [ ] **MESC IM 生成**：基于细粒度标注结果，使用5套模板
- [ ] 部署服务器批处理任务

#### 2.2 中文数据集 IM 生成
- [ ] **RECAP 中文 IM 生成**：5,154条样本
- [ ] 设计中文版5套模板
- [ ] 部署后台批处理任务（建议 DeepSeek-v3 降低成本）
...
#### 3.3 DPO 对齐
- [ ] 提取 Hard Case（心理活动和回复明显割裂、配合咨询师的失败样本）
- [ ] 构造 Positive/Negative 对
- [ ] 惩罚"治愈幻觉"与"逻辑断裂"
...
- [x] **ESConv/MESC 细粒度标注完成** ✅ (2026-03-26)
  - [x] ESConv：19,372条 utterance 级别二分类 → 5,609条阻抗 (PsyFIRE 13类，100%准确率) + 13,763条合作 (CBS 7类)
  - [x] MESC：18,436条 utterance 级别二分类 → 1,608条阻抗 (PsyFIRE 13类，100%准确率) + 16,828条合作 (CBS 7类)
  - [x] 质量修复：清洗带#注释样本(43条)、官方prompt重跑(111条修复)、规则清洗(100%完成)
  - [x] 关键发现：ESConv A2占76.2%（情绪化模拟），MESC A1+C1占83.2%（理性真实）
  - [x] 输出文件：`esconv_resistance_fine_full.json`, `mesc_resistance_fine.json` 等
- [ ] **准备 RECAP 模型权重对接**
  - [ ] 签署数据共享协议并获取模型权重
  - [ ] 评估 RECAP 模型对 ESConv/MESC 的 Zero-shot 性能
- [ ] **人工复核质检**
  - [ ] 对自动生成的标签进行抽样人工 Review，形成高质量 Ground Truth
